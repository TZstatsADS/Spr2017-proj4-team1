---
title: "HMRF_EM Report for J_Martin"
author: "group 1"
date: "April 15, 2017"
output: pdf_document
---
...
We will only use dataset 6 for testing!
...
we define a set of constraint functions C={c1, c2, ..., cm}. The constraint function is a Boolean-valued function and is de- fined as follow:
c(p,p )=⎧⎪1 if pi andpj satisfytheconstraintcl lij⎨
⎪⎩0 otherwise
```{r}
...
Objective Function 
Based on HMRF, the conditional distribution of the re- searcher labels y given the observation x (paper) has :maxp(Y|X)∝minfobj =∑∑{D(xi,xj)I(li ≠lj)∑[wkck(pi,pj)]} ij ck∈C
+∑D(x,y )+logZ
```{r}
...
Hmrf-em algorithm
#initiialization
For initialization of our EM framework, we first cluster publica- tions into disjoint groups based on the constraints over them, i.e. if two publications have a constraint, then they are assigned to the same researcher. Therefore, we first get λ groups. If λ is equal to the actual researcher number k, then these λ groups are used as our initial assignment. If λ< k, we choose another (k-λ) clusters by random perturbations of the global centroid. If λ>k, we cluster the nearest group until there are only k groups left.
```{r}
#Initialization
...
we write the E-Setp function and M-Step function respectively.
```{r}
...
this is the values we need for dataset6
```{r}
t=2
w2 = 0.7
w6 = w2^t
...
Sys.time()-ss
#!!Caution:If the above code can not run, pls first run the part before(while) in the EM algorithm.then test label[[d]].It's a bug might due to too many functions. Some functions can not pass the correct value to the other.
