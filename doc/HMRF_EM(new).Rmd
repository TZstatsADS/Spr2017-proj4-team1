...
cluster_number <- vector(length=length(data.files))
paper_id <- vector(length=length(data.files))
...
for (j in 1:length(data.files)) {
  ...
  paper_id[j]<- as.vector(data_unlist[which(names(data_unlist)=="paper_id")])
  #calculate actual researchers size for each list
  cluster_number[j]=length(unique(as.vector(data_unlist[which(names(data_unlist)=="author_id")])))
...
for (i in 1:length(data.files)){
  ...
  M[[i]] <- cbind(rbind(Mp,Map),rbind(Mpa,Ma))
}
#coauthor constraint
...
#Assume tao=2, then we get 2-coauthor constraint
...
Objective Function 
```{r}
for(s= 1:length(data.files)){
##HMRF Ojective function for list s
# Data set X: dtm_train_tfidf[[i]];Weights:w1=0.7,w2=0.7^2; 
#c1(pi,pj)=1 if and only M1[i,j]=1; and c2(pi,pj)=1 if and only M2[i,j]=1;
#D(xi,xj) is the [i,j]element of the cosine similarity matrix D; 
#label Li is obtained from the E-Step of EM algorithm; yli is the centroid point obtained from the M-step
#k indicates real researcher number
k=cluster_number[s]
w1 <- 0.7
w2 <- 0.7^2
n <- length(data_list[[s]])
objf <-function(w1=0.7,w2=0.7^2,M1=M1,M2=M2,n=n,D,L){
  I <- matrix(rep(0,n^2),n,n)
  #Asumme Label(cluster) is a vector with length=n
  for(i in 1:k){
    #calculate centroid of k cluster
      yli <-mean(dtm_train_tfidf[[i]][which(Label=k)])
      ## A Question Here: How to calculate D(xi,yli)
      ##suppose we already have a vector to represent the distance between the point and the cluster centroid :d= D(xi,yli)
  }
  ...
  for(i in 1:n){
    for(j in 1:n){
      I[i,j] <- ifelse(Label[i]==label[j],1,0)
      f1 <- D[i,j]*I[i,j]*(w1*M1[i,j]+w2*M2[i,j])
      f <-f+f1
    }
    f2 <- f2+ d[i]
  }
