...
```{r}
EM_algorithm = function(l, d, t=2, w2 = 0.7, w6 = w2^t ,tau, eta, max.iter){
...
for (d in 1:length(data.files)){
  label[[d]]=EM_algorithm(l=l, d=d, w2 = 0.7, w6 = w2^2 ,tau=0.01, eta=0.01, max.iter=3)[[1]]
}
```
...
performance_EM<-list(1:length(data.files))
for (d in 1:length(data.files)){
  
  table_1<-table(author_id[[d]])
  table_EM<-table(l)
  k<-length(table(author_id[[d]]))
  per<-permutations(k,k,1:k)
  num<-NULL
  for (i in 1:nrow(per)){
    num[i]<-sum(abs(table_EM[per[i,]] - table_1))
  }

  min_per<-per[which.min(num),]
  result_EM<-l


  for (i in 1:k){
     result_EM[result_EM == i] <- min_per[i]+100
  }
  result_EM< -result_EM- 100

  
  performance_EM[[d]] <- performance_statistics(matching_matrix(author_id[[d]],result_EM))
  compare_df <- data.frame(Name=query.list,
                         precision=performance_EM[[d]][1],
                         recall=performance_EM[[d]][2],
                         f1=performance_EM[[d]][3],
                         accuracy=performance_EM[[d]][4])
