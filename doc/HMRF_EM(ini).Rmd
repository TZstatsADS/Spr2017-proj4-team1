...
constraint2 <-function(i,j,d){
  return(length(intersect(data_list[[d]][[i]]$coauthor_list,data_list[[d]][[j]]$coauthor_list))!=0)
}
```
...
Initialization <-function(A,d){
  ...
  while(length(index)>0){
    ...
    for (j in index){
    i=index[1]
    if(constraint2(i,j,name)=0 & constraint6(i,j,name)!=0){index=index[-j]}#remove index already belong to a cluster
    else{l[j]=lambda}
    }
  }
  ...
  if(lambda<k){
      ...
      for(i in 1:lambda){
        #calculate centroids
    X = as.matrix(dtm_train[[d]])[l==i,] #subset the Xi to the one with the label
    Y[i,] = col_sums(X)
    ...
    while(k-lambda>0){
      lambda=lambda+1
      Y[lambda,]=glo_Y+rnorm(n=dim(dtm_train[[d]])[2],mean=0,sd=(3)^(1/3))
    }
    l=Estep(A, l, Y, d, k , t, w2 = 0.7, w6 = w2**t)
    }
    ...
    X = as.matrix(dtm_train[[d]])[l==i,] #subset the Xi to the one with the label
    Y[i,] = col_sums(X)
    Y[i,] = Y[j,]/(sqrt((t(Y[j,]) %*% A %*% Y[j,])[1]))
      }
      #calculate distances between cluster j and cluster h
      dis=matrix(NA,lambda,lambda)
      for(j in 1:lambda){
        for(h in (j+1):lambda){
    ...
    k1=which.min(dis,arr.id=TRUE)[1]
    k2=which.min(dis,arr.id=TRUE)[2]
...
#E-step
Estep = function(A, l, Y, d, k , w2 = 0.7, w6 = 0.7^2){
...
#Mstep
Mstep = function(A, l, Y, name, d, k ,w2 = 0.7, w6 = 0.7^2, eta){
  #update Y first:
  for(j in 1:k){
...
```{r}
EM_algorithm = function(initial A, initial clustering l,ini){
  while(not converged i.e. objective function moving){
    call E function updating l
    call M function updating Y and A
  }
}
```
