...
objective = function(A, l, Y, d, w2 = 0.7){
  obj = 0
  for(i in dim(dtm_train_tfidf[[d]])[1]){
    for(j in dim(dtm_train_tfidf[[d]])[1]){
      obj = obj + (l[i]==l[j]) * distance(A,as.matrix(dtm_train_tfidf[[d]])[i,],as.matrix(dtm_train_tfidf[[d]])[j,]) * (w2 * constraint2(i,j,d) )
    }  
  }
  for(i in dim(dtm_train_tfidf[[d]])[1]){
